...
def process_html(directory):
    ...
    ids = []
    if os.path.isfile(f"./csv/{city}_complete.csv"):
      with open(f"./csv/{city}_complete.csv","r") as csv_file:
          reader = csv.DictReader(csv_file)
          for row in reader:
              ids.append(row['post_id'])
    
    with open(f"./csv_dumps/{city}_csv_dump.csv","w") as csvfile:
      ...
      for filename in tqdm(os.listdir(directory), desc=f"Processing html from {directory}..."):
          ...
          if repost_of:
              ...
              if os.path.exists(json_path):  # if its file already exists, modify the repost_dates
                  ...
                  if isinstance(data["repost_dates"],str): data['repost_dates']=[data['repost_dates']]
