...
#SBATCH --output=../outfiles/distilroberta/seattle_e2_test.out # Path for output must already exist
#SBATCH --error=../outfiles/distilroberta/seattle_e2_test.err  # Path for error must already exist
#SBATCH --job-name="testing e2 seattle"

module purge
cd /projects/p31502/projects/craigslist
source ./venv_craig_env/bin/activate
cd ./models/distilroberta
python distilroberta.py test seattle 2
