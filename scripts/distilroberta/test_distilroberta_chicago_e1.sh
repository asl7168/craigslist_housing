...
#SBATCH --output=../outfiles/distilroberta/chicago_e1_test.out # Path for output must already exist
#SBATCH --error=../outfiles/distilroberta/chicago_e1_test.err  # Path for error must already exist
#SBATCH --job-name="testing chicago e1"

module purge
cd /projects/p31502/projects/craigslist
source ./venv_craig_env/bin/activate
cd ./models/distilroberta
python distilroberta.py test chicago 1
