...
#SBATCH --output=../outfiles/distilroberta/chicago_e2_train.out # Path for output must already exist
#SBATCH --error=../outfiles/distilroberta/chicago_e2_train.err  # Path for error must already exist
#SBATCH --job-name="distilling chicago, double epoch"

module purge
cd /projects/p31502/projects/craigslist
source ./venv_craig_env/bin/activate
cd ./models/distilroberta
python distilroberta.py train chicago 2
