...
#SBATCH --output=../outfiles/distilroberta/chicago_e2_test.out # Path for output must already exist
#SBATCH --error=../outfiles/distilroberta/chicago_e2_test.err  # Path for error must already exist
#SBATCH --job-name="testing chicago e2"

module purge
cd /projects/p31502/projects/craigslist
source ./venv_craig_env/bin/activate
cd ./models/distilroberta
python distilroberta.py test chicago 2
